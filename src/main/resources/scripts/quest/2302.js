        ...
        if(mode == 0 && type > 0) {
            if (status != 3) {
                ...
                if (qm.canHold(4032375, 1)) {
                    qm.sendNext("可以。那样的话，我就给你介绍去蘑菇王国的路线。#b在射手村西边的传送门附近，#b在射手村西边的出租车附近，#k你会发现#b有一颗大树#k.跳到顶端进入这颗大树, 然后从左下角的入口进入#b<主题地图:蘑菇城堡>#k.那是蘑菇城堡的入口。时间不多了！");
                } else {
            ...
            if (status == 0) {
                qm.sendAcceptDecline("既然你已经取得了工作进展，而且看起来已经准备好了。我有件事想请你帮忙。你愿意听吗?");
            } else if (status == 1) {
                qm.sendNext("#b蘑菇王国#k目前处于混乱状态.蘑菇王国位于射手村附近,聪明的国王以爱好和平为特色. 最近，他开始感到不舒服，所以他决定任命他唯一的女儿#b碧欧蕾塔#k. 从那以后一定发生了什么事情，王国才处于目前的状态。");
            } else if (status == 2) {
                qm.sendNext("不知到底发生了什么事情，不过好像遇到了很大的麻烦，所以最好还是去一趟，看看有什么事情可以帮忙。如果是勇士的话，应该能解救陷入危机的蘑菇王国。我已经写好了#b推荐书#k，快去蘑菇王国吧。去找#b警卫队长#k就可以了。\r\n\r\n#fUI/UIWindow.img/QuestIcon/4/0#\r\n#v4032375# #t4032375#");
            } else if (status == 3) {
...
function end(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            qm.sendNextPrev("嗯...好吧.既然这封信是转职教官写的，我想你就是那个人。我很抱歉没有早点做自我介绍。我是负责保护蘑菇王国的#b警卫队长#k.如你所见，在士兵小组的保护下这个临时藏身处暂时还很安全，但事实上我们的处境可能很严峻，欢迎你来到蘑菇王国。");
        } else if (status == 2) {
            qm.gainItem(4032375, -1);
            qm.gainExp(6000);
